...
def test_op_total_no_changes():
    assert op_total(MockDataProgram(100)) == 100, "Total should not change upon viewing it"
    assert op_total(MockDataProgram(1.2)) == 1.2, "Total should not change upon viewing it"
    assert op_total(MockDataProgram(-1.2)) == -1.2, "Total should not change upon viewing it"
    assert op_total(MockDataProgram(float('Inf'))) == float('Inf'), "Total should not change upon viewing it"
    assert op_total(MockDataProgram(-float('Inf'))) == -float('Inf'), "Total should not change upon viewing it"
    assert isnan(op_total(MockDataProgram(float('Nan')))), "Total should not change upon viewing it"

def test_op_credit_adds_amount():
    assert op_credit(MockDataProgram(100), "50") == 150
    assert op_credit(MockDataProgram(0), "0") == 0
    assert op_credit(MockDataProgram(-10), "10") == 0
    assert op_credit(MockDataProgram(1.5), "2.5") == 4.0

def test_op_credit_invalid_amount():
    with pytest.raises(InvalidAmountError):
        op_credit(MockDataProgram(100), "abc")
        assert False, "Should raise InvalidAmountError"

def test_op_credit_large_numbers():
    assert op_credit(MockDataProgram(0), str(5_000_000_000_000_000)) == 5_000_000_000_000_000, (
...
def test_op_debit_subtracts_amount():
    assert op_debit(MockDataProgram(100), "50") == 50
    assert op_debit(MockDataProgram(10), "10") == 0
    assert op_debit(MockDataProgram(5.5), "2.5") == 3.0

def test_op_debit_insufficient_funds():
    # Balance should not change with insufficient funds
    assert op_debit(MockDataProgram(10), "20") == 10
    assert op_debit(MockDataProgram(0), "1") == 0

def test_op_debit_invalid_amount():
    with pytest.raises(InvalidAmountError):
        op_debit(MockDataProgram(100), "xyz")
        assert False, "Should raise InvalidAmountError"

def test_intentional_bug_op_credit_negative_treated_as_positive():
    assert op_credit(MockDataProgram(100), "-12") == 112, (
...
def test_intentional_bug_op_debit_negative_treated_as_positive():
    assert op_debit(MockDataProgram(100), "-12") == 88, (
    "This bug (negative number treated as positive) "
    "has been here long enough that people might depend on it."
    )